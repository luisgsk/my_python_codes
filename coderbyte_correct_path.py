...
def calcpath(str):
    move_map = {"d": -1, "u": 1, "l": -1, "r": 1}
    q_count = str.count("?")
    move_combinations = itertools.product(move_map.keys(), repeat=q_count)
    ...
    while True:
        ...
        possible_route = str.replace("?", "{}").format(*moves)
        for move in possible_route:

            if move in "lr":
                current_x += move_map.get(move)
            elif move in "du":
                current_y += move_map.get(move)

            if current_x < 0 or current_x == 0 or current_x > 5:
                break
            elif current_y < 0 or current_y == 0 or current_y > 5:
                break

        # Check route
        if (current_x, current_y) == (5, 1):
            return possible_route


# print(calcpath("???rrurdr?"))
